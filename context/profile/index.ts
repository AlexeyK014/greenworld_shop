...
// import { handleJWTError } from "@/lib/utils/errors";
// import { IEditUserEmailFx, IEditUsernameFx, IUploadUserAvatarFx, IVerifyCodeFx, IVerifyEmailFx } from "@/types/profile";
// import axios from "axios";
// import { createDomain } from "effector";
// import toast from "react-hot-toast";
// import api from '@/api/apiInstance';
...
// export const uploadUserAvatarFx = profile.createEffect(async ({ jwt, formData }: IUploadUserAvatarFx) => {
//   try {
//     const { data } = await axios({
//       method: 'post',
//       url: '/api/users/avatar',
...
//     if (data?.error) {
//       const result: { image: string } = await handleJWTError(data.error.name, {
//         repeatRequestMethodName: 'uploadUserAvatarFx',
//         payload: { formData }
//       });
//       return result;
...
// // setEdit - чтобы после конца запроса мы открывали логику редактирования имени
// export const editUsernameFx = profile.createEffect(
//   async ({ jwt, setEdit, name }: IEditUsernameFx) => {
//     try {
//       const { data } = await api.patch(
...
//       toast.success('Имя сохранено')
//       setEdit(false) // скрываем логику редактирования
//       localStorage.setItem('auth', JSON.stringify(data.tokens))
//       updateUsername(name)
//       return data
//     } catch (error) {
...
// // при нажатие на галочку в личном кабинете вызываем этот эффект
// // происходит проверка на бэке
// export const verifyEmailFx = profile.createEffect(
//   async ({ jwt, email }: IVerifyEmailFx) => {
//     try {
//       const { data } = await api.post(
...
//         //   return
//         // }
//         const result: { codeId: string } = await handleJWTError(data.error.name, {
//           repeatRequestMethodName: 'verifyEmailFx',
//           payload: { email }
//         });
//         return result;
...
// // code - который юзер ввел из инпут
// export const editUserEmailFx = profile.createEffect(
//   async ({ jwt, setEdit, code, codeId, email }: IEditUserEmailFx) => {
//     try {
//       const {data} = await verifyCodeFx({ code, codeId, jwt })
