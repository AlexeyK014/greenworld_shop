...
export const updateUserPassword = passwordRestore.createEvent<IUpdateUserPasswordFx>()
...
export const updateUserPasswordFx = passwordRestore.createEffect(
  async ({ email, password }: IUpdateUserPasswordFx) => {
    try {
      const { data } = await api.post( // делаем запрос
        '/api/users/password-restore',
        {
          email, password
        }
      )
