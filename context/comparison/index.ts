...
export const addProductToComparison = comparison.createEvent<IAddProductToComparisonFx>();
...
export const addProductsFromLSToComparison =
  comparison.createEvent<IAddProductsFromLSToComparisonFx>();
export const deleteProductFromComparison = comparison.createEvent<IDeleteComparisonItemsFx>();
...
export const addProductToComparisonFx = createEffect(
  async ({ jwt, setSpinner, ...payload }: IAddProductToComparisonFx) => {
    try {
      ...
      //  повторяем запрос при протухание токена
      if (data?.error) {
        const newData: { newComparisonItem: IComparisonItem } = await handleJWTError(
          data.error.name,
          {
...
export const getComparisonItemsFx = createEffect(async ({ jwt }: { jwt: string }) => {
  try {
    ...
    if (data?.error) {
      const newData: IComparisonItem[] = await handleJWTError(data.error.name, {
...
export const addProductFromLSToComparisonFx = createEffect(
  async ({ jwt, comparisonItems }: IAddProductsFromLSToComparisonFx) => {
    try {
      ...
      //  повторяем запрос при протухание токена
      if (data?.error) {
        const newData: { comparisonItems: IComparisonItem[] } = await handleJWTError(
          data.error.name,
          {
