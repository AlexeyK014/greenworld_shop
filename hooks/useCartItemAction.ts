...
export const useCartItemAction = (cartItem: ICartItem) => {
  // стэйт для спинера для удаления товара
  const [deleteSpinner, setDeleteSpinner] = useState(false);
  const [count, setCount] = useState(+cartItem.count); // состояние для счетчика
  ...
  const handleDeleteCartItem = () => {
    if (!isUserAuth()) {
      ...
      deleteProductFromLS(
        cartItem.clientId,
        'cart', // key корзины
        setCartFromLS, // передаём эвент для обновления стора
        setShouldShowEmpty,
        'Удалено из корзины!',
      );
      return;
