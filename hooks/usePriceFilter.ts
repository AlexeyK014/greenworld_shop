...
import { checkPriceParam, formatPrice, getSearchParamUrl } from '@/lib/utils/common';
...
export const usePriceFilter = () => {
  ...
  // обновляем фильтры согласно price
  useEffect(() => {
    ...
    if (priceFromParam && priceToParam) {
      if (checkPriceParam(+priceFromParam) && checkPriceParam(+priceToParam)) {
        ...
        setPriceInfo(
          // для селекта, чтобы появлялась информация о диапазоне
