...
const CartPopup = forwardRef<HTMLDivElement, IWrappedComponentProps>(({ open, setOpen }, ref) => {
  ...
  const spinner = useUnit(getCartItemsFx.pending); // спинер для загрузки товара в корзине
  const currentCartByAuth = useGoodsByAuth($cart, $cartFromLs); // товары корзины
