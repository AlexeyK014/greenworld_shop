...
const ProductPage = ({ productId, category }: IProductPageProps) => {
  ...
  useEffect(() => {
    if (breadcrumbs) {
      ...
      // обращаемся к предпоследней хлебной крошке
      breadcrumbs.children[breadcrumbs.children.length - 2].children[0].textContent = (
        translations[lang].breadcrumbs as { [index: string]: string }
      )[category];

      // динамическое изменение последней хлебной крошки
      // вместо последней крошки показываем либо спинер либо называние
      lastCrumb.textContent = productSpinner ? translations[lang].common.loading : product.name;
    }
